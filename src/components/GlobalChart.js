...
import colorsFromCss from "../styles/_settings.scss";
...
const GlobalChart = ({ coinsData }) => {
  ...
  const colorPicker = (number) => {
    if (number >= 20) {
      return colorsFromCss.color1;
    } else if (number >= 5) {
      return colorsFromCss.green2;
    } else if (number >= 0) {
      return colorsFromCss.green1;
    } else if (number >= -5) {
      return colorsFromCss.red1;
    } else if (number >= -20) {
      return colorsFromCss.red2;
    } else {
      return colorsFromCss.black2;
    }
  };
